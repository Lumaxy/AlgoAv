/*****************************************************************************/
/* C.PORQUET                 genere_graphe_1.c                 D�cembre 2007 */
/*                     G�n�rateur automatique de graphes                     */
/*                  Les sommets sont num�rot�s � partir de 1                 */
/*****************************************************************************/
...
{ int i;
  ...
  g->matrice = (float **)calloc(g->nbsom+1,sizeof(float*));
  for (i=1;i<=g->nbsom;i++)
      { g->matrice[i] = (float *)calloc(g->nbsom+1,sizeof(float));
        /* for (j=1; j<=g->nbsom; j++) g->matrice[i][j] = 0.;
	   inutile avec calloc */
      }
}
/*****************************************************************************/
...
void init_ens(int dim, t_ens *ens)
	{int i;
	 ...
	 ens->pere =(int *)calloc(dim+1, sizeof(int));
	 ens->nbelements = ens->nbclasses = dim;
	 for (i=1;i<=ens->nbelements;i++) ens->pere[i]=-1;
	 }
/*****************************************************************************/
